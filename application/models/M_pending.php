...
class M_pending extends CI_Model
{
	...
	public function join_table(){
        ...
        WHEN ra.approve_status = 3 AND rd.status = 2 AND ds.status IS NULL AND p.status IS NULL AND b.status IS NULL THEN 'Packaging-New'
        ...
        WHEN ra.approve_status = 3 AND rd.status = 2 AND ds.status = 2 AND p.status IS NULL AND b.status IS NULL THEN 'BOM-New'
        WHEN ra.approve_status = 3 AND rd.status = 2 AND ds.status = 2 AND p.status = 2 AND b.status = 1 THEN 'BOM-Pending'
        WHEN ra.approve_status = 3 AND rd.status = 2 AND ds.status = 2 AND p.status = 2 AND b.status = 2 THEN 'Receive-New' 
