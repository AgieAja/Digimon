...
class M_finish extends CI_Model
{
	...
	public function join_table(){
		...
		return $query = $this->db->query("SELECT rd.*,rh.request_no,rh.request_date,c.name,ds.sakura_version_no,b.movex_filter_master,b.sap_filter_master,r.created_at as r_created_at
	...
	public function report(){
		...
		return $query = $this->db->query("SELECT rd.*,rh.request_no,rh.request_date,c.name,ds.sakura_version_no,b.movex_filter_master,b.sap_filter_master,b.remark as b_remark,r.created_at as r_created_at
				FROM request_details as rd
				LEFT JOIN request_headers as rh ON rd.request_header_id=rh.request_header_id
				LEFT JOIN customers as c ON rh.customer_code=c.customer_code
				LEFT JOIN drawing_specs as ds ON rd.request_detail_id=ds.request_detail_id
